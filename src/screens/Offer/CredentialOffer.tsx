import { updateAnonCredsModuleV0_3_1ToV0_4 } from "@aries-framework/anoncreds/build/updates/0.3.1-0.4";
import {
...
const CredentialOffer = ({ navigation, route }) => {
  ...
  useEffect(() => {
    updateCredentialFormat(credential_offer_id);
  }, []);

  async function updateCredentialFormat(id: string) {
    const formatData: GetCredentialFormatDataReturn<CredentialFormat[]> =
      await agent.agent.credentials.getFormatData(id);
    ...
    if (formatData.offerAttributes) {
      ...
      setCredentialFormatData(newCredentialFormatData);
    } else {
  ...
  async function declineOffer() {
    await agent.agent.credentials.declineOffer(credential_offer_id);
  }

  async function storeToWallet() {
      ...
      <View>
        ...
        <View style={styles.card}>
          <View style={styles.credentialHeader}>
            ...
            <View style={styles.rightContent}>
              <Text style={{ fontWeight: "bold", fontSize: 16 }}>Degree</Text>
            </View>
          </View>
